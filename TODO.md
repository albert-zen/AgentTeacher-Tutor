...
**现状：** 当前模型配置主要包含 provider/baseURL/model/apiKey 等字段；`/api/llm-status` 仅返回通用配置状态，未显式描述模型是否支持 vision。视觉能力是否可用只能依赖模型名约定或运行时报错，前端无法在配置阶段准确决定是否展示图片上传入口。

**目标：** 在模型配置阶段就能明确模型能力（至少包含 `vision`），并由服务端统一下发能力信息；前端据此启用/禁用图片相关 UI 与消息格式，避免“可见但不可用”或错误调用。

**工程要点：**
- 扩展 LLM 配置结构（配置文件与类型）：新增能力字段（如 `capabilities: { vision: boolean }`），支持手动配置与默认推断。
- 更新 `packages/server/src/routes` 的 LLM 状态与更新接口返回：将能力字段并入 `llm-status` 响应，保证客户端可直接消费。
- 更新 `packages/client/src/api/client.ts` 的 `LLMStatus` 类型与设置页读取逻辑，展示“视觉能力已启用/未启用”状态。
- 在 `ChatPanel` 图片上传入口（视觉输入功能落地时）使用该能力字段做显式门控：无 `vision` 时隐藏或禁用并给出提示。
- 为服务端能力解析与客户端门控补测试：覆盖“vision=true/false”两种配置路径。
...
待完成:
  ...
  视觉输入支持
    ├→ 依赖多模态 LLM（模型层面）
    └→ 依赖: LLM 模型能力协商（显式视觉能力开关）

  LLM 模型能力协商（显式视觉能力开关）（Tier 3）
    └→ 支撑: 视觉输入支持
